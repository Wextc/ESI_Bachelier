...
n, qui représente la taille de la liste à créer,

nmin, la valeur minimale possible,

nmax, la valeur maximale possible.

La liste en compréhension parcourt range(n) afin de répéter l’opération n fois. À chaque itération, la fonction random.randint(nmin, nmax) génère un nombre entier aléatoire compris entre nmin et nmax, bornes incluses. Chaque valeur générée est ajoutée automatiquement à la liste. La liste complète est ensuite retournée par la fonction.
...
Cette fonction prend en argument une liste de caractères et la transforme en une liste d’entiers représentant la position de chaque lettre dans l’alphabet. La lettre 'a' est associée à la valeur 0, 'b' à la valeur 1, et ainsi de suite jusqu’à 'z' qui correspond à 25.

La fonction commence par vérifier que l’argument fourni est bien une liste. Ensuite, elle parcourt chaque élément de cette liste et vérifie qu’il s’agit d’une chaîne de caractères composée d’un seul caractère, et que ce caractère est une lettre minuscule non accentuée comprise entre 'a' et 'z'. Ces vérifications permettent de garantir que la fonction ne traite que des données valides.

La conversion est réalisée grâce à la fonction ord(), qui renvoie le code Unicode d’un caractère. Comme ord('a') vaut 97 et ord('z') vaut 122, soustraire ord('a') au code du caractère permet d’obtenir un entier compris entre 0 et 25 correspondant à la position de la lettre dans l’alphabet.
...
Voici une explication en texte fluide, ligne par ligne, de la fonction moyenne

Cette fonction a pour objectif de calculer la moyenne des éléments d’une liste, à condition que cette liste soit valide et ne contienne que des nombres.

Tout d’abord, la fonction commence par vérifier que l’argument fourni est bien une liste. Si ce n’est pas le cas, elle renvoie False, ce qui permet d’éviter des erreurs si on lui passe un type incorrect comme un entier ou une chaîne de caractères.

Ensuite, la fonction vérifie que la liste n’est pas vide. Calculer une moyenne sur une liste vide n’a pas de sens, car il serait impossible de diviser par zéro. Si la liste est vide, la fonction renvoie donc également False.

Une variable total est ensuite initialisée à zéro. Elle servira à stocker la somme de tous les éléments de la liste.

La fonction parcourt ensuite la liste à l’aide d’une boucle for basée sur les indices. À chaque itération, elle vérifie que l’élément courant est bien un nombre, c’est-à-dire un entier ou un nombre à virgule flottante. Si un élément n’est pas de ce type, la fonction s’arrête immédiatement et renvoie False, garantissant ainsi que la moyenne ne sera calculée que sur des données numériques valides.

Si l’élément est correct, sa valeur est ajoutée à la variable total. Une fois tous les éléments parcourus et additionnés, la fonction calcule la moyenne en divisant la somme totale par le nombre d’éléments dans la liste.
...
La fonction commence par créer une liste vide appelée res. Cette liste servira à stocker le résultat final, c’est-à-dire les éléments présents à la fois dans l1 et dans l2.

Ensuite, la fonction parcourt tous les éléments de la première liste l1. À chaque itération, la variable i représente un élément de cette liste. Pour chaque élément, la fonction vérifie deux conditions. La première condition, i in l2, permet de savoir si cet élément est également présent dans la seconde liste l2. Cela garantit que l’élément est commun aux deux listes. La seconde condition, i not in res, permet de vérifier que cet élément n’a pas déjà été ajouté à la liste résultat. Cette vérification empêche l’apparition de doublons dans la liste retournée.

Si ces deux conditions sont satisfaites, l’élément est ajouté à la liste res à l’aide de la méthode append. Une fois que tous les éléments de l1 ont été examinés, la fonction retourne la liste res, qui contient uniquement les éléments communs aux deux listes, chacun apparaissant une seule fois.
...
Cette fonction a pour objectif de supprimer toutes les occurrences d’un mot donné dans une liste et de compter combien de suppressions ont été effectuées.

La fonction commence par initialiser une variable compteur à zéro. Cette variable servira à mémoriser le nombre de fois où le mot est supprimé de la liste.

Ensuite, une boucle while est utilisée avec la condition mot in ma_liste. Tant que le mot à supprimer est présent dans la liste, la boucle continue de s’exécuter. À chaque passage dans la boucle, la méthode remove(mot) supprime la première occurrence du mot dans la liste. Comme une suppression a été effectuée, le compteur est incrémenté de 1.

La boucle s’arrête automatiquement dès que le mot n’est plus présent dans la liste, ce qui garantit que toutes les occurrences ont été supprimées. À la fin de la fonction, la valeur du compteur est retournée. Cette valeur correspond exactement au nombre total de suppressions réalisées.
